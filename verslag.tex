...
\section{Data}
We hebben voor elk algoritme 2 verschillende soorten tests uitgevoerd. Ten eerste hebben we voor gekeken naar de werking op de hele set van genres. De data van deze tests is te zien in de tabellen 8, 10, 12, 14, 16, 18, 20, 22 en 24. In de cel (Country, Folk) staat het percentage van de Country nummers die als Folk zijn gelabeld door het algoritme. Daarnaast hebben we gekeken naar de werking  op alle subsets van 2 genres. De data daarvan is te vinden in de tabellen 7, 9, 11, 13, 15, 17, 19, 21 en 23. In de cel (Country, Folk) staat het percentage van de Country nummers dat goed wordt geclasificeerd als de genres Country en Folk worden getest. Voor iedere soort test hebben we ook gekeken naar 3 verschillende grootes van de trainingsset namelijk 10, 25 en 40. De grote van de testset staat in de naam van de tabel samen met het algoritme en het soort test ("All" voor alle genres en "Pairs" voor de subset test). Om de data makelijkerer te kunnen analyseren hebben we tabelen met gemiddelden gemaakt per grote van de trainingsset en per soort test.

\section{Analyse}
Uit onze data concluderen we dat het naive bayes het beste werkt op de tests met alle genres. Het SVM algoritme werkt net iets slechter en het kNN algoritme werkt het minst goed. Voor de grote van de trainingsset geeft 40 het de beste  resultaten, gevolgd door 25 en de tests met trainingssize van 10 gaf de slechtste resultaten. 
